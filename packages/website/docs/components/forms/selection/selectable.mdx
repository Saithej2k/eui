...
**EuiSelectable** aims to make the pattern of a selectable list (with or without search) consistent across implementations. It is the same concept used in [EuiComboBox](./combo-box.mdx) and [EuiFilterGroup](../../navigation/buttons/filter-group.mdx).
...
Set `isInvalid` to mark both the search field and options list as invalid for assistive technologies. Pair the invalid state with an error message that explains how to correct the selection.

```tsx interactive
import React, { useState } from 'react';
import { EuiFormRow, EuiSelectable, EuiSelectableOption } from '@elastic/eui';
...
export default () => {
  ...
  const isInvalid = !options.some(({ checked }) => checked === 'on');

  return (
    <EuiFormRow
      label="Moons"
      isInvalid={isInvalid}
      error={isInvalid ? 'Select at least one moon' : undefined}
    >
      <EuiSelectable
        aria-label="Moons"
        searchable
  ...
  return (
    <>
      ...
      <EuiSelectable
        ...
        onChange={onChange}
        style={{ width: 300 }}
        listProps={{ bordered: true, paddingSize: 's' }}
      >
        {(list) => list}
      </EuiSelectable>
    </>
...
export default () => {
  const [options, setOptions] = useState<EuiSelectableOption[]>([
    ...
    {
      prepend: <EuiIcon type="warning" color="warning" aria-label="Note!" />,
      label:
        ...
        {textWrap === 'wrap' && (
          <EuiFlexItem grow={false} css={{ alignSelf: 'center' }}>
            <EuiText color="danger" size="s">
              <EuiIcon type="warning" /> Wrapping text requires disabling
...
export default () => {
  ...
  return (
    <EuiSelectable
      ...
      options={options}
      listProps={{ style: { maxInlineSize: 400 } }}
      onChange={(newOptions) => setOptions(newOptions)}
    >
      {(list) => list}
    </EuiSelectable>
...
In a testing environment, virtualization will prevent all options from being rendered to the DOM, which can cause test selectors to fail. You can disable it with  `listProps.isVirtualized` set to `false`.

When testing `EuiSelectable` directly, you can pass `listProps={{ isVirtualized: false }}`.

Alternatively, you can mock `EuiSelectable` globally:

```js
jest.mock("@elastic/eui", () => {
  const { EuiSelectable } = jest.requireActual("@elastic/eui");
  return {
    ...jest.requireActual("@elastic/eui"),
    EuiSelectable: (props: EuiSelectableProps) => {
      return (
        <EuiSelectable
          {...props}
          listProps={{ ...props.listProps, isVirtualized: false }}
        />
      );
    },
  };
  ...
  return (
    <>
      ...
      <EuiSelectable
        ...
        listProps={{
          isVirtualized,
          rowHeight: useCustomContent ? 57 : undefined,
          showIcons: false,
        }}
